...
class ViewDesc {
    ...
    setSelection(anchor, head, view, force = false) {
        ...
        if ((domSel.extend || anchor == head) && !(brKludge && gecko$1)) {
...
function inOrNearComposition(view, event) {
    ...
    if (safari$1 && Math.abs(Date.now() - view.input.compositionEndedAt) < 500) {
    ...
    if (!view.composing) {
        ...
        if (state.selection instanceof TextSelection &&
            (state.storedMarks ||
                ...
                chrome$1 && windows$1 && selectionBeforeUneditable(view))) { // Issue #1500
    ...
    constructor(view, handleDOMChange) {
        ...
        this.observer = window.MutationObserver &&
            new window.MutationObserver(mutations => {
                ...
                else if (safari$1 && view.composing && mutations.some(m => m.type == "childList" && m.target.nodeName == "TR")) {
    ...
    flush() {
        ...
        if (added.some(n => n.nodeName == "BR") &&
            (view.input.lastKeyCode == 8 || view.input.lastKeyCode == 46 ||
                chrome$1 && (view.composing || view.input.compositionEndedAt > Date.now() - 50) &&
...
function readDOMChange(view, from, to, typeOver, addedNodes) {
    ...
    let mkTr = (base) => {
        ...
        if (parse.sel) {
            ...
            if (sel && !(chrome$1 && view.composing && sel.empty &&
...
function autoLinkPlugin(schema) {
...
const markdownToTable = (schema, text) => {
